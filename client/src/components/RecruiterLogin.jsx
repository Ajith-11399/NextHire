...
const RecruiterLogin = () => {
    ...
    const { setShowRecruiterLogin, serverUrl, setCompanyToken, setCompanyData } = useContext(AppContext);
        ...
        try {
            ...
            if (state === "Login") {
                ...
                const { data } = await axios.post(
                    serverUrl + '/api/company/login', {email, password}
                );
                ...
                const { data } = await axios.post(
                    serverUrl + '/api/company/register', formData
                );
    ...
    return (
        ...
        <div className='absolute top-0 left-0 right-0 bottom-0 z-20 backdrop-blur-sm bg-black/30 flex items-center justify-center'>
            ...
            <form onSubmit={onSubmitHandler} className='relative bg-white p-10 rounded-xl text-slate-500'>
                ...
                <button type='submit' className={`bg-blue-600 w-full text-white py-2 rounded ${state === 'Login' ? 'mt-0 bg-gradient-to-r from-violet-600 to-fuchsia-500 text-white' : 'mt-4 bg-gradient-to-r from-green-600 to-emerald-500' } `}>
