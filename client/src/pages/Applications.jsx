        ...
        <>
            ...
            <div className='container px-4 py-10 min-h-[65vh] 2xl:px-20 mx-auto mt-28 mb-10'>
                ...
                <div className='flex gap-2 mt-3 mb-6'>
                    {
                        ...
                        ? <>
                            <label className='flex items-center' htmlFor="resumeUpload">
                                <p className='bg-blue-100 text-blue-600 text-blue px-4 py-2 rounded-lg mr-2'>Select Resume</p>
                                <input id='resumeUpload' onChange={ (e)=> e.target.files[0] } accept='application/pdf' type="file" hidden />
                                <img src={assets.profile_upload_icon} alt="" />
                            </label>
                            <button onClick={ ()=> setIsEdit(false) } className='bg-green-100 text-green-600 border border-green-400 rounded-lg px-4 py-2'>Save</button>
                        </>
                ...
                <table className='min-w-full bg-white border rounded-lg'>
                    ...
                    <tbody>
                        {jobsApplied.map((ele, ind) => true ? (
                            <tr>
                                <td className='py-3 px-4 fex items-center gap-2 border-b'>
                                    <img className='w-8 h-8' src={ele.logo} alt="" />
                                    {ele.company}
                                </td>
                                <td className='py-2 px-4 border-b'>{ele.title}</td>
                                <td className='py-2 px-4 border-b max-sm:hidden'>{ele.location}</td>
                                <td className='py-2 px-4 border-b max-sm:hidden'>{moment(ele.date).format('ll')}</td>
